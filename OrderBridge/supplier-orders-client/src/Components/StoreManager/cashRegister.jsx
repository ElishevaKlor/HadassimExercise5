...
import { useAddStoreGoodMutation } from "../storeGoodsApiSlice";
...
const CashRegister = () => {
  ...
  const [addStoreGoods, { isLoading, isSuccess, isError }] = useAddStoreGoodMutation();
  ...
  useEffect(() => {
    if (isSuccess) {
      toast.current.show({
        ...
        detail: 'המוצר נוסף בהצלחה לקופה',
        life: 3000,
      });
      reset(); 
    }
  ...
  return (
    <div className="form-container" style={{ paddingTop: "50px" }}>
      ...
      <div className="form-card">
        <h2 className="form-title">הוספת מוצר לקופה</h2>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {isError && <small className="p-error">מוצר זה כבר קיים בקופה</small>}
        </form>
      </div>
      <Toast ref={toast} />
    </div>
